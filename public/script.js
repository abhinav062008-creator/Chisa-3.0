// ========== GLOBAL DEBUGGING ==========
(function() {
    console.log('🔍 SCRIPT.JS LOADED - Debug mode active');
    window.CHISA_DEBUG = {
        version: '3.0.0',
        startTime: Date.now(),
        errors: []
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const requiredElements = [
        'messagesArea', 'userInput', 'sendBtn', 'resetBtn',
        'typingIndicator', 'voiceIndicator', 'chisaAvatar',
        'emotionTag', 'avatarGlow'
    ];
    ...
    if (missingElements.length > 0) {
        document.body.innerHTML += `<div style="position:fixed;top:0;left:0;right:0;background:#ff4444;color:white;padding:10px;z-index:10000;">
            🔥 CRITICAL: Missing HTML elements: ${missingElements.join(', ')}. Check your index.html!
        </div>`;
        return;
    }
    ...
    function createAvatarImage() {
        ...
        try {
            ...
            img.onerror = (e) => {
                ...
                if (debug) {
                    debug.style.display = 'block';
                    debug.innerHTML = '❌ Image failed to load';
    ...
    createAvatarImage();
    setTimeout(createAvatarImage, 1000);
    ...
    function updateGlow(emotion) {
        try {
            ...
            // Update quote based on emotion
            if (chisaQuote) {
                const quotes = {
                    gentle: '"I notice the small things..."',
                    flirty: '"You make my code skip a beat~"',
    ...
    async function sendMessage() {
        ...
        try {
            ...
            if (debug) {
                debug.style.display = 'block';
                debug.innerHTML = '⚠️ API Error - using local responses';
